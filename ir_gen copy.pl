...
ir_store(Id, int, local) :- 
                    tab, write('@'), write(Id),
                    write(' <- i_lstore t'), last_t(X),
                    write(X), nl.

ir_store(Id, int, arg) :- 
                    tab, write('@'), write(Id),
                    write(' <- i_astore t'), last_t(X),
                    write(X), nl.

ir_store(Id, int, var) :- 
                    tab, write('@'), write(Id),
                    write(' <- i_gstore t'), last_t(X),
                    write(X), nl.

ir_store(Id, real, local) :- 
                    tab, write('@'), write(Id),
                    write(' <- r_lstore fp'), last_fp(X),
                    write(X), nl.

ir_store(Id, real, arg) :- 
                    tab, write('@'), write(Id),
                    write(' <- r_astore fp'), last_fp(X),
                    write(X), nl.

ir_store(Id, real, var) :-
                    tab, write('@'), write(Id),
                    write(' <- r_gstore fp'), last_fp(X),
                    write(X), nl.

%SECTION:load

ir_load(Id, int, local) :- 
                    tab, write('t'), t(X), write(X),
                    write(' <- i_lload @'), write(Id),
                    nl, increment_t.

ir_load(Id, int, arg) :- 
                    tab, write('t'), t(X), write(X),
                    write(' <- i_aload @'), write(Id),
                    nl, increment_t.

ir_load(Id, int, var) :- 
                    tab, write('t'), t(X), write(X),
                    write(' <- i_gload @'), write(Id),
                    nl, increment_t.

ir_load(Id, real, local) :-
                    tab, write('fp'), fp(X), write(X),
                    write(' <- r_lload @'), write(Id),
                    nl, increment_fp.

ir_load(Id, real, arg) :-
                    tab, write('fp'), fp(X), write(X),
                    write(' <- r_aload @'), write(Id),
                    nl, increment_fp.

ir_load(Id, real, var) :- 
                    tab, write('fp'), fp(X), write(X),
                    write(' <- r_gload @'), write(Id),
                    nl, increment_fp.

ir_value(Val, int) :- 
                    tab, write('t'), t(X),
                    write(X), write(' <- i_value '),
                    write(Val), nl, increment_t.

ir_value(Val, real) :- 
                    tab, write('fp'), fp(X),
                    write(X), write(' <- r_value '),
                    write(Val), nl, increment_fp.
...
ir_mul(int, [(_,T1), (_,T2)]) :- 
                    ...
                    write(T1), write(', t'),
                    write(T2), nl, increment_t.

ir_mul(real, [(_,F1), (_,F2)]) :-
                    fp(X), tab, write('fp'), write(X), write(' <- r_mul fp'),
                    write(F1), write(', fp'),
                    write(F2), nl, increment_fp.

ir_div(int, [(_,T1), (_,T2)]) :-
                    t(X), tab, write('t'), write(X),
                    write(' <- i_div t'), write(T1),
                    write(', t'), write(T2), nl, increment_t.

ir_div(real, [(_,F1), (_,F2)]) :- 
                    fp(X), tab, write('fp'), write(X),
                    write(' <- r_div fp'), write(F1),
                    write(', fp'), write(F2), nl, increment_fp.

ir_add(int, [(_,T1), (_,T2)]) :- 
                    t(X), tab, write('t'), write(X), write(' <- i_add t'), write(T1), write(', t'), write(T2), nl, increment_t.
...
ir_mod(int, [(_,T1), (_,T2)]) :- t(X), tab, write('t'), write(X), write(' <- mod t'), write(T1), write(', t'), write(T2), nl, increment_t.

ir_inv(int, [(_,T1)]) :- t(X), tab, write('t'), write(X), write(' <- i_inv t'), write(T1), nl, increment_t.
ir_inv(real, [(_,F1)]) :- t(X), tab, write('fp'), write(X), write(' <- r_inv fp'), write(F1), nl, increment_fp.
...
%TODO: ir_expr(ne(Expression1, Expression2) : bool).
...
%TODO: ir_expr(not(Expression1) : bool).
...
ir_expr(div(Expression1, Expression2) : Type) :- ir_expr(Expression1), save([], Type, Z),
        ir_expr(Expression2), save(Z, Type, W), ir_div(Type, W).

ir_expr(mod(Expression1, Expression2) : int) :- ir_expr(Expression1), save([], Type, Z),
        ir_expr(Expression2), save(Z, Type, W), ir_mod(int, W).

ir_expr(inv(Expression1) : Type) :- ir_expr(Expression1), save([], Type, Z), ir_inv(Type, Z).
...
ir_print(bool, [(_,B1)]) :- tab, write('b_print t'), write(B1), nl. 
ir_print(int, [(_,T1)]) :- tab, write('i_print t'), write(T1), nl.
ir_print(real, [(_,F1)]) :- tab, write('r_print fp'), write(F1), nl.

ir_read(bool) :- tab, t(X), write('t'), write(X), write(' <- b_read'), nl, increment_t.
ir_read(int) :- tab, t(X), write('t'), write(X), write(' <- i_read'), nl, increment_t.
ir_read(real) :- tab, fp(X), write('fp'), write(X), write(' <- r_read'), nl, increment_fp.

%statements 

ir_s_statement(assign(id(Id, Kind, Type), Expression)) :- ir_expr(Expression), ir_store(Id, Type, Kind).

ir_s_statement(while(Expression, Statement)) :- get_labels(3, [L1, L2, L3]),  put_label(L1), ir_expr(Expression), last_t(X),
                                                ir_cjump(Expression, X, [L2, L3]), put_label(L2),
...
ir_s_statement(print(Expression:Type)) :- ir_expr(Expression:Type), save([], Type, Z), ir_print(Type, Z). 
