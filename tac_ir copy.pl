...
read_std(end_of_file, []).
read_std(X, [X|Y]) :- read_term(Z, []), read_std(Z, Y).

read_std(Y) :- read_term(X, []), read_std(X, Y).

start(Name) :- reset_t, reset_fp, reset_l, open(Name, read, In), read_terms(In, X), close(In), ir_ast_list_process(X), !.
start() :- reset_t, reset_fp, reset_l, read_std(X), ir_ast_list_process(X), !.